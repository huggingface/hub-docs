...
When a 429 error occurs, the SDK automatically parses the `RateLimit` header to extract the exact number of seconds until the rate limit resets, then waits precisely that duration before retrying. This applies to all HTTP calls made through the library, including file downloads, repository operations, and paginated API calls.
