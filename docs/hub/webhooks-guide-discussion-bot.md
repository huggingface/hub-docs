...
Here's a short guide on how to use Hugging Face Webhooks to build a bot that replies to Discussion comments on the Hub with a response generated by an LLM of your choice via the Chat Completions API.
...
This is the coolest part: using the [Chat Completions API](https://huggingface.co/docs/inference-providers/chat-completions), we call a free model (`meta-llama/Llama-3.1-8B-Instruct`) with a system prompt and the user's comment. The model generates a reply, which we extract from `choices[0].message.content`.
