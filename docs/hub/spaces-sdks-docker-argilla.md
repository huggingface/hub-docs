...
If you want to play with the UI without installing anything, check this [live demo](https://huggingface.co/spaces/argilla/live-demo) is powered by Hugging Face Docker Spaces, exactly the same technology you'll learn in this guide. To login into the UI use: username: argilla, password: 1234.
...
And create your first dataset by logging it into Argilla using your endpoint URL and (optionally) `API_KEY`:
...
This section provides some details about what's done under the hood for setting up the Docker Space.
...
3. Launch Elasticsearch, setup the `API_KEY` and `PASSWORD` if available, and launch the Argilla service. This done in the `start.sh` script.

If you are looking for a more advanced configuration of Argilla, setting up users and workspaces you can check the [Argilla Demo Docker Space codebase](https://huggingface.co/spaces/argilla/live-demo/tree/main).
