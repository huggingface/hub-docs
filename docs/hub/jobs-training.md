...
- **One file.** Declare its dependencies in a `# /// script` header and run `hf jobs uv run train.py`. The TRL section shows a header.
- **A project folder**, with local imports, a `pyproject.toml` or config files. `hf jobs uv run` uploads only the script file, so mount the folder instead. The mount is read-only, so the command copies the project to a writable directory and runs it there, as you would locally:
  ...
  `uv run` installs dependencies from `pyproject.toml` or the script header, and local imports and relative paths work unchanged. See [Local directories](./jobs-configuration#local-directories).
